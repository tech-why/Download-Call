(function ( $ ) {
    ...
    $.fn.searchProduct = function( options ){
        ...
        function searchData( text ) {
            $.ajax({
                url: 'productController.do?search1',
                type: 'POST',
                data: {
                    ...
                    if( data.obj.length > 0 ){
                        $.each(data.obj, function( i, item ) {
                            var context = {
                                ...
                                itemOrigPrice: item.productPrice,
//                                itemSalePrice: item.itemSalePrice,
        ...
        function getTemplate( fn ){
            $.get('productController.do?display1?'+Math.random(), function( html ){
                fn( html );
            });
        };
    };
    ...
    $.fn.pagination = function( options ) {
        ...
        function getData( elems, pageNo ) {
            var catalogId = $.productParam({'param': 'catalogId'}),
            ...
            if( catalogId != null ){
                ajaxParam.catalogId = catalogId;
            };
            if( shopId != null ){
                ajaxParam.shopId = shopId;
            };
            // $.getOpenId({
            //     onSuccess: function(){
                    // $( '#shoppingCart' ).cart({
                        ...
                        getTemplate(function( html ) {
                            ...
                            $.each(data.obj, function( i, item ) {
                                var context = {
                                    ...
                                    itemImageUrl: item.productImageUrl,
                                    itemDetailsUrl:'/jeewx/productItemController.do?display&productId='+item.id+'&open_user_id='+settings.userOpenid +'&shopId='+settings.shopId,
                                    itemSaleCount:item.saleCount,
                                    stockNum: data.obj[0].remainAmount,
                                    productUnit:data.obj[0].productUnit
                ...
                getTemplate(function( html ) {
                    ...
                    $.each(data.hotProduct, function( i, item ) {
                        ...
                        var context = {
                            ...
                            itemImageUrl: item[0].productImageUrl
                        };
                        var template = Handlebars.compile( source );
                        var html = template( context );
    ...
    $.fn.cart = function( options ){
        ...
        function delCart( elems ){
            $.ajax({
                ...
                data: {
                    'method': 'deleteshop',
                    'goodGroupId': $(elems).attr('cartid'),
                    'shopid': $(elems).parents('li').attr('itemid')
                },
                dataType: 'json'
            }).done(function( data ){
        ...
        function updateCart( fn ){
            ...
            $( '#cartList' ).find( 'li' ).each(function( i ) {
                ...
                _temp = {
                    'id': _tempId,
                    'groupNum': $( this ).find( '.js-count' ).val(),
        ...
        function getCart( elems ){
            var groupIdStr = '[]';
            if( $.hysCookie({'type':'getCookie','cname':'hyscart'}) ){
                groupIdStr = decodeURIComponent( $.hysCookie({'type':'getCookie','cname':'hyscart'}) );
            };
            $.ajax({
                url: '/mall/success.html',
                type: 'POST',
                data: {
                    method: 'combineShopCar',
                    groupIdStr: groupIdStr,
                    code: $.productParam({'param': 'code'})
                },
                dataType: 'json'
            }).done(function( data ){
                ...
                if( data.success == 'success' ){
                    ...
                    $.ajax({
                        ...
                        getTemplate(function( html ) {
                            ...
                            $.each(data.shopCarList, function( i, item ) {
                                var context = {
                                    cartId: item.goodGroupId,
                                    itemId: item.id,
                                    itemName: item.groupName,
                                    itemCount: item.groupNum,
                                    itemImageUrl: item.itemImageUrlSmall,
                                    itemSalePrice: item.itemSalePrice,
                                ...
                                if( item.shopState == 1 ){
                                    ...
                                    allPrice += (item.itemSalePrice * 100) * item.groupNum;
                                };
            ...
            function getTemplate( fn ){
                $.get('static/template/cart_list.html?' + Math.random(), function( html ){
                    fn( html );
                });
            };
        };
        ...
        function checkOut( elems ){
            $( elems ).click(function(){
                var checkOutUrl = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxab144e9dd399684a&redirect_uri=http://w.ehaoyao.com/haoyao/wchatGoodGroup.do?method=getList&response_type=code&scope=snsapi_base&state=123#wechat_redirect',
                    //checkOutUrl = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxe453c29113157ed1&redirect_uri=http://124.202.153.75/haoyao/wchatGoodGroup.do?method=getList&response_type=code&scope=snsapi_base&state=id#wechat_redirect',
                    stateParam = [],
                ...
                if( selectItem.length > 0 ){
                    updateCart(function(){
                        ...
                        //     type: 'GET',
                        //     data: {
                        //         'method': 'payShopNumber'
                        //     },
                        //     dataType: 'json'
                        // }).done(function( data ){
                        //     console.log(data)
                        //     if( parseInt(data.shopCarNum) > 0 ){
    ...
    $.fn.productDetail = function( options ){
        ...
        function getInfo( elems ){
            ...
            //     });
            // }else{
                ajaxParam.productId = $.productParam({'param': 'productId'});
                getData(ajaxParam);
            // };
...
//                        var imageUrl = [];
                        var imageUrlString = '';
                        var context = {
                            itemId: data.obj[0].id,
                            itemName: data.obj[0].productName,
                            itemOrigPrice: data.obj[0].normalPrice,
                            itemSalePrice: data.obj[0].productPrice,
                            itemImageUrl: data.obj[0].productImageUrl,
                            description: data.obj[0].productDescription,
                            stockNum: data.obj[0].remainAmount,
                            saleCount: data.obj[0].saleCount,
                            productUnit:data.obj[0].productUnit
                        };
//                        imageUrl.push(data.goodGroup.itemImageUrl);
//                        if( data.wchatGoodDetails ){
//                            context.itemImageUrlArray = [];
//                            $.each(data.obj[0], function( i, item ){
...
//                        };
                        ...
                        // });
                        imageUrlString += '<img src="'+ data.obj[0].productImageUrl +'" class="img-responsive" alt="">';
