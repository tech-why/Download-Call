...
if($('#typeid').val()==""){
	...
	$.ajax({  
	    url :"/jeewx/submitProblemController.do?submitproblem",
	    type:"post",    //数据发送方式  
	    ...
	    data:{
			...
			typeId : $('#typeid').val(),
			problemMessage : $('#problemMessage').val()
		
	    }, 
	   //要传递的数据；就是上面序列化的值  
	    success: function(data) {
	   
	    	window.location.href="/jeewx/submitProblemController.do?submitsuccess&open_user_id=${open_user_id}&problemid="+data.obj;
		} 
	});
}
function div(){
	...
	<div id = "cartList" data-count="${cartListLength}" class="container" style="padding-left:0px; padding-right:0px;">	
		 ...
		 <div class="panel listaddr panel-default" id="li-283">
		 	...
		 	<div class="panel-heading">
                	...
                	故障地址
                    <a class="edit-addr" href="addressController.do?display&open_user_id=${open_user_id}&app_code=${app_code}"><i class="fa fa-pencil-square-o"> 更换</i></a> 
            </div>
                <div class="panel-body">
                    <address id="shopAddress" data-id = "address.id">
					
                      <h5><span class="text-muted">地 址：</span>${school.schoolName }${tbbudding.getBuilddingName() }${address.houseCode } </h5>
        ...
        <div class="panel">
          ...
          <div id="invoicePanel" class="panel-body" style="padding-top:15px; position:relative">
						<textarea class="form-control" rows="3" style="text-align: left;" id="problemMessage" name="problemMessage"></textarea>
						<span id="tipSaddress" class="requiredSpan" style="color: red;"></span>
          </div>
          
        </div>
      
       <p style="padding:0 15px;">   <button id="submitProblemButton" type="button" class="btn btn-o2o  btn-block"  id="zhifu1" onClick="baoxiu()" style="margin-bottom:15px;"  >报修故障</button></p>
  ...
  <div id="Selectdrugs" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
        ...
        <div class="tabs-content">
                ...
                <div class="panel panel-default panelA">
             	...
             	<c:forEach var="type" items="${typeproblemlist }">
							...
							<h4 class="panel-title">
							  <a style="text-decoration:none">
								<p  id="${type.id }"drugid="${type.id }" >
									...
									<span class="js-drugName">${type.typeName }</span>
