...
<!-- 	<script src="http://crypto-js.googlecode.com/svn/tags/3.1.2/build/rollups/md5.js"></script>
	...
	<meta charset="utf-8">
		<title style="font:30px #ffffff">微信校园超市</title>
...
function aa(biaoshi){
	if(vilidateform())
	{
		...
		$("#submitOrderButton").attr("disabled","disabled");//再改成disabled  
		
		$.ajax({  
		    url :"/jeewx/generateOrderController.do?validateOrder",
		    type:"post",    //数据发送方式  
		    ...
		    success: function(data) {
		    		 ...
		    		 if(data.success==true){	    		
		    			...
		    			$("#submitOrderButton").removeAttr("disabled");//要变成Enable，JQuery只能这么写       
		    			alert(data.msg);
		    		} 
				} 
		});
	...
	<div id = "cartList" data-count="${cartListLength}" class="container" style="padding-left:0px; padding-right:0px;">	
		<c:forEach var="cart" items="${cartList}">
			<div class="panel">
				<div class="panel-body">
					<div class="media">
					    <a class="pull-left" href="#" style="width:80px;">
					    	<img class="media-object img-responsive" src="${cart.tbMallProduct.productImageUrl}" alt="">
					    </a>
					    <div class="media-body">
					    	<h4 class="media-heading">${cart.tbMallProduct.productName}</h4>
					    	<p style="margin-top:5px; color:#f15a28; font-size:14px">
					    		微信价
					    		<strong class="text-danger" style="font-size:18px; color:#f15a28;">￥
					    			<em id="price" style="font-style:normal;">
					    				${cart.tbMallProduct.productPrice}
					    			</em>
					    		</strong>
					    	</p>
							<p>数量：<span style="color:#f15a28">${cart.count}</span></p>
					    </div>
					</div>
				</div>
			</div>
		 ...
		 <div class="panel listaddr panel-default" id="li-283">
		 	...
		 	<div class="panel-heading">
                	...
                	收货地址
                    <a class="edit-addr" href="addressController.do?display&open_user_id=${open_user_id}&app_code=${app_code}&shopId=${shopId}"><i class="fa fa-pencil-square-o"> 更换</i></a> 
            </div>
                <div class="panel-body">
                    <address id="shopAddress" data-id = "address.id">
					
                      <h5><span class="text-muted">地 址：</span>${addressDetail} </h5>
                      <h5><span class="text-muted">姓 名：</span>${address.contact}</h5>
                      <h5><span class="text-muted">电 话：</span>${address.mobileNumber}</h5>
                    </address>            
                </div>
           </div>    
		 	...
		 	<div class="panel-heading">
                ...
                <div class="panel-body">
                    <address>
                      ...
                      <h5><span class="text-muted">日&nbsp&nbsp&nbsp期：</span> <span id= "deliverDate" > </span> </h5>
                      <h5><span class="text-muted">时间段：</span> <span id= "deliverTime"> </span> </h5>
                    </address>            
                </div>
           </div>    
        ...
        <div class="panel">
          <div class="panel-heading">
		    ...
		    <span style="font-size:16px">用户留言</span>
          </div>
		...
		<div class="alert alert-danger" style="background:#fff; border-color:#e6e5e5; color:#34495e; padding:10px 0 ">
		
			 
      <input id="caocao_pic" type="hidden" value="static/images/selectno.png"/>
		
			<div style="padding:8px 15px 0 15px;"><p>商品价格:<span style="color:#f15a28">￥${cartTotalAmount}</span></p>
			
			<p>满${freeTranseportAmount}包邮￥${transportFee}</p>						
			<p>总计价格:<span style="color:#f15a28">￥${totalAmount}</span></p>
			
			</div>
		</div>
      
       <p style="padding:0 15px;">   <button id="submitOrderButton" type="button" class="btn btn-o2o  btn-block"  id="zhifu1" onClick="aa(1)" style="margin-bottom:15px;"  >货到付款</button></p>
...
<!--选择药品-->
  <div id="Selectdrugs" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true" style="display: none;">
	    <div class="modal-dialog">
		    <div class="modal-content">
		        <div class="modal-header" style="border-bottom:2px solid #09b173">
		        	...
		        	<h4 class="modal-title"><i class="icon-uniE614" style="color:#41c27f; padding-right:5px; vertical-align:-2px; font-size:20px;"></i>送货时间</h4>
		        </div>
	          <div class="modal-body" style="padding:10px 0 0 0">
		        	<div id="tabs" class="tabs">
					<ul id="selectDrugsTabs">
				    <li class="current">
							<div class="tabs-itmes">今天<span></span></div>
					  </li>
						<li >
							<div class="tabs-itmes">明天<span></span></div>
						</li>
        ...
        <div class="tabs-content">
                ...
                <div class="panel panel-default panelA">
                  ...
                  <div class="js-collapsePanel ">
					   ...
					   <c:forEach var="workTime" items="${workTimeList }">
					   	<c:if test="${workTime.todayOrTomorow == 'today' }">
							...
							<h4 class="panel-title">
							  <a style="text-decoration:none">
								<p id="${workTime.id}" drugid="21">
									...
									<span class="js-drugName">${workTime.startTime}--${workTime.endTime}</span>
            ...
            <li style="padding:20px;"  data-date = "tomorrow">
              <div class="panel-group">
                 <div class="panel panel-default panelA">
                  <div class="js-collapsePanel ">
                   	<c:forEach var="workTime" items="${workTimeList }">
